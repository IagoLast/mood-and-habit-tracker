...
export class ScoresRepository {
  ...
  async upsertWithClient(client: PoolClient, params: { userId: string; date: string; score: number }) {
  ...
  async deleteWithClient(client: PoolClient, userId: string, date: string) {
    await client.query(
      'DELETE FROM daily_scores WHERE user_id = $1 AND date = $2',
      [userId, date]
    );
